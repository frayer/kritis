...
1. Set up Helm:
    ...
    [Install Helm v3](https://helm.sh/docs/intro/install/) for your platform
...
1. `GenericAttestationPolicy` is set, but no attestation exists for the pod in
   ...
   You'll be prompted to enter a password to protect the private key generated by `gpg`.  Press `<Enter>` to skip password protection for this demonstration.  If you choose to password protect the key, note the additional literal value needed on the Kubernetes Secret created when running `./no_attestation.sh` on the `kubectl create secret ...` line.
