...
helm install grafeas https://storage.googleapis.com/grafeas-charts/repository/grafeas-charts-0.1.6.tgz --set secret.enabled=true --set certificates.enabled=true --set certificates.ca="$(cat ca.crt)" --set certificates.cert="$(cat grafeas.crt)" --set "certificates.key=$(cat grafeas.key)" --set service.type="LoadBalancer" --set service.port=443
